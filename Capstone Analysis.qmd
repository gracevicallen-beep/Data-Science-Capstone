...
supporting strong operational performance (AUC = 0.92).

GAMs therefore provide a modeling framework that balances accuracy,
interpretability, and accountability, making them well-suited for
effective and trustworthy fraud detection.
...
**Model Overview** The model uses a logistic binomial Generalized
Additive Model (GAM) to predict fraud probability in financial
transactions. GAMs are particularly well-suited for fraud detection
because they estimate the log-odds of fraud as a smooth, additive
function of predictors, allowing the model to capture non-linear
relationships while maintaining interpretability—crucial for
understanding which factors drive fraud risk. In this case, the GAM
predicts the probability that a transaction is fraudulent by analyzing
features such as transaction amount, account balance, card age, and risk
score. The smooth functions in the GAM highlight threshold effects and
curved risk patterns, showing how each variable contributes to fraud
probability beyond simple linear trends. Model diagnostics, including
the confusion matrix and ROC curve, confirm strong classification
performance, while the ability to visually display nonlinear variable
effects ensures the model remains transparent and compliant for
operational decision-making.

**Primary Goal** To identify fraudulent transactions by analyzing
patterns in transaction characteristics and assigning fraud
probabilities using a logistic binomial Generalized Additive Model
(GAM). This model estimates the log-odds of fraud as a smooth, additive
function of key predictors—most notably Risk_Score and
Transaction_Amount—allowing it to capture nonlinear relationships while
remaining interpretable. The goal is to maximize fraud detection
(sensitivity) while minimizing false alarms (false positives), which is
critical in financial fraud prevention where accuracy and transparency
are essential for operational decision-making and regulatory compliance.

**Key Predictive Features** The GAM analyzes several variables to
predict fraud: Risk_Score (Primary Driver) Strongest predictor with
highly significant impact (p ≈ 0) Shows a threshold effect: fraud
probability remains relatively stable at low-to-mid risk scores but
increases sharply around 0.75 The smooth, non-linear relationship
captures complex patterns in the data Transaction_Amount Positive
relationship: higher transaction amounts are associated with increased
fraud risk Clear upward trend indicates larger transactions warrant more
scrutiny Account_Balance and Card_Age Show minimal impact on fraud
prediction (relatively flat curves) Included for completeness but
contribute less to the model's decisions
...
interpretation. The basis-dimension results indicate that all three
smooth terms—Transaction_Amount, Account_Balance, and Card_Age—have
k-indices very close to 1 with non-significant p-values. This suggests
that the spline basis for each smooth is adequate and neither under- nor
over-smoothed.

The residual diagnostics provide additional support for model adequacy.
The residuals-versus-fitted plot does not show any strong structure or
curvature, which indicates that the additive form of the model is
appropriate for the data. The Q–Q plot shows some deviations in the
extreme tails, which is expected in a heavily imbalanced fraud dataset,
but there is no major departure from the theoretical quantiles overall.
The histogram of residuals displays the expected skew for a binomial
response, and the ACF plot shows no signs of meaningful autocorrelation,
supporting the assumption of residual independence.

Inspection of the smooth functions shows that none of the three
continuous predictors exhibit unusually complex or erratic patterns. The
estimated degrees of freedom are close to 1, indicating that the
relationships between each predictor and the log-odds of fraud are
essentially linear once the other terms are accounted for. The smooths
are stable across the observed ranges, with wide confidence intervals
where data are sparse, which is typical and acceptable.

Taken together, the diagnostics suggest that the GAM is well-specified
for this dataset. The smoothing parameters appear appropriate, the
residual behavior is consistent with model assumptions, and there is no
evidence of problematic dependence or concurvity among predictors.
Overall, the model satisfies the key assumptions needed for reliable
inference and provides a stable foundation for interpreting how
transaction-related features relate to fraud probability.
...
# Fit GAM model quietly
gam_model <- suppressMessages(suppressWarnings(
  gam(
    Fraud_Label ~
      Merchant_Category +
      Is_Weekend +
...
# 1. Basis dimension check (captured quietly)
gam_check_results <- suppressMessages(suppressWarnings(
  capture.output(gam_check_results <- gam.check(gam_model))
))

# 2. Diagnostic plots saved to file (no console pop-ups)
par(mfrow = c(2,2))
...
# 3. Concurvity
conc_vals <- concurvity(gam_model, full = TRUE)
```
...
*Table 4* presents a GAM using only categorical predictors to examine
associations with fraud. Each estimate represents the log-odds
difference relative to the reference category. Standard errors indicate
uncertainty, and confidence intervals crossing 1 indicate no significant
difference.
...
log-odds of fraud when Risk_Score is zero. The smooth term
\$s(\\text{Risk_Score})\$ captures the nonlinear relationship between
Risk_Score and fraud probability. The effective degrees of freedom for
the smooth term is approximately 9, allowing flexibility in modeling
nonlinear changes. Its high significance indicates a strong association
with fraud.
...
This project explored the use of Generalized Additive Models (GAMs) to
identify variables predictive of fraudulent activity in financial
transactions. GAMs provided a strong balance between interpretability
and predictive performance, allowing us to model nonlinear relationships
between features such as Risk_Score, Transaction_Amount, Card_Age, and
prior transaction history. Smooth term visualizations highlighted how
each variable influenced fraud probability, aligning with the project
aim of determining key predictive factors.

The model demonstrated solid classification performance. Risk_Score
emerged as the dominant predictor, with fraud probability sharply
increasing around a score of 0.75, while Transaction_Amount showed a
moderate positive effect. Account_Balance and Card_Age had minimal
impact. The confusion matrix indicated that out of the test
transactions, 2,318 fraudulent transactions were correctly identified
(true positives) and 10,102 legitimate transactions were correctly
classified (true negatives), while 77 legitimate transactions were
incorrectly flagged (false positives) and 2,502 fraudulent transactions
were missed (false negatives). The ROC curve had an area under the curve
(AUC) of 0.73, reflecting good discriminative ability.

Several limitations were noted. The dataset is synthetic and relatively
evenly distributed, which reduced the availability of strong predictors
and may have dampened variable effects. GAMs are sensitive to class
imbalance, potentially affecting recall for rare fraud cases, and
categorical variables with high cardinality may require preprocessing to
prevent overfitting. Additionally, while GAMs are interpretable, more
complex machine learning methods like gradient boosting could achieve
higher raw predictive accuracy.

Despite these limitations, GAMs effectively identified and quantified
the influence of transaction characteristics on fraud probability,
providing interpretable insights for operational decision-making. Future
work could integrate GAMs into real-time fraud detection systems,
combine them with ensemble methods for higher accuracy, and leverage
streaming or temporal data to capture evolving fraud patterns. Overall,
this analysis demonstrates the utility of GAMs in detecting fraud while
maintaining transparency and trustworthiness for stakeholders.
