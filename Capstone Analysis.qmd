...
-   Observations are independent, meaning one transaction does not
    affect another. Each case stands on its own.
...
-   Predictors are assumed to not be too strongly correlated with each
    other so the model can estimate each variable’s effect clearly.
...
**Plot 3 (Pingping - need to have title for types of graphs)**
```{r}
library(mgcv)
library(pROC)
library(caret)
...
train <- df[train_idx, ]
test  <- df[-train_idx, ]
## 3. Fit GAM (FIXED: smart k + select=TRUE)
gam_mod <- gam(
  Fraud_Label ~
    s(Transaction_Amount,           k = 50) +
    Transaction_Type +
    s(Account_Balance,             k = 50) +
    Device_Type +
    Location +
    Merchant_Category +
    IP_Address_Flag +
    Previous_Fraudulent_Activity +
    s(Daily_Transaction_Count,     k = 10) +
    s(Avg_Transaction_Amount_7d,   k = 50) +
    s(Failed_Transaction_Count_7d, k = 5)  +
    Card_Type +
    s(Card_Age,                    k = 50) +
    s(Transaction_Distance,        k = 50) +
...
# - true labels: test$Fraud_Label
# - predicted probabilities: test_prob

# Step 1: Convert probabilities to class predictions
threshold <- 0.5
test_pred <- factor(ifelse(test_prob > threshold, 1, 0), levels = c(0,1))

# Step 2: Generate confusion matrix
cm <- confusionMatrix(test_pred, test$Fraud_Label, positive = "1")
print(cm)

# Step 3: Prepare data for heatmap-style plot
...
p_cm <- ggplot(cm_table, aes(x = Reference, y = Prediction, fill = Freq)) +
  ...
  scale_fill_gradient2(low = "#2c7bb6", mid = "#ffffbf", high = "#d7191c",
                       midpoint = mean(cm_table$Freq), name = "Count") +
  labs(
    title = "Confusion Matrix",
    subtitle = paste0("Threshold = 0.5 | Acc = ", round(cm$overall["Accuracy"], 4)),
  ...
  coord_fixed()
print(p_cm)
ggsave("Confusion_Matrix.png", p_cm, width = 7, height = 6, dpi = 300)
...
cat("Files saved: ROC_Curve.png, Confusion_Matrix.png\n")
...
data$pred_prob <- predict(gam_model, type = "response")  # Generate predicted probabilities


```

The plots illustrate how each predictor variable influences the response
