...
**Model Overview**
The model uses a Generalized Additive Model (GAM) to predict fraud probability in financial transactions. GAMs are particularly well-suited for fraud detection because they can capture non-linear relationships while maintaining interpretability—crucial for understanding which factors drive fraud risk.
In this case, the GAM predicts the probability that a transaction is fraudulent by analyzing features such as transaction amount, account balance, card age, and risk score. The smooth functions in the GAM show how each variable contributes to fraud risk.Model diagnostics, including the confusion matrix and ROC curve, its ability to visually display how each variable influences fraud risk ensures the model remains interpretable and transparent for decision-making and compliance.
