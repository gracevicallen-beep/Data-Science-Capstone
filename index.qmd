...
*Figure 4* shows the outcome variable Fraud_Label shows a moderate class
imbalance, with 67.87% non-fraud cases and 32.13% fraud cases. Although
the imbalance is noticeable, it is not severe enough to require
resampling or other corrective techniques prior to modeling. We will
assess model performance using metrics that are less sensitive to class
imbalance, AUC and the confusion matrix, rather than relying on accuracy
alone.
...
fraud_data %>%
  ...
  rename(Count = n) %>%
  kable(caption = "Figure 4. Breakdown of Fraud_Label")
```
...
The smooth term analysis in *Figure 8* establishes a hierarchy for
...
*Figure 9* shows a coefficient plot displaying the estimated odds ratios
from the GAM. In this plot, each dot represents the estimated odds of
fraud for a specific category level relative to the reference level,
while the vertical lines (whiskers) show the 95% confidence intervals.
...
ggplot(tidy_cat, aes(x = reorder(term, OR), y = OR)) +
  ...
  coord_flip() +
  labs(
    title = "Figure 9. Categorical Variables and Fraud",
...
knitr::kable(gam_summary, caption = "Table 5. Parametric Terms in GAM Model")
```
...
knitr::kable(smooth_summary, caption = "Table 6. Smooth Terms in GAM Model")
```
...
*Figure 10* is a generalized additive model (GAM) used to examine how
...
*Figure 11* shows the confusion matrix for the GAM Fraud Detector,
...
*Figure 12* shows the ROC Curve for the GAM Fraud Detector, which
...
-   Risk_Score – the dominant predictor, with fraud probability sharply
    increasing around a score of 0.75, showing a clear threshold effect.
...
cases; however, the imbalance in our dataset is not extreme, so
evaluation metrics (including AUC and the confusion matrix) should still
provide an accurate assessment of performance. Real-world fraud datasets
would likely have a greater imbalance and would therefore need to
account for it. Additionally, while GAMs are interpretable, more complex
machine learning methods such as gradient boosting could achieve higher
predictive a
